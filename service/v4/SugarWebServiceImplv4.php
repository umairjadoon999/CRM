...
class SugarWebServiceImplv4 extends SugarWebServiceImplv3_1
{
    ...
    public function login($user_auth, $application = null, $name_value_list = array())
    {
        ...
        if ($isLoginSuccess) {
            ...
            function_exists('openssl_decrypt') && $authController->authController->userAuthenticateClass == "LDAPAuthenticateUser"
            ...
            if (
                $authController->login($user_auth['user_name'], md5($user_auth['password']), array('passwordEncrypted' => true))
     ...
     * ***
     */
    public function portal_user_login($session, $parameters)
    {
        global  $beanList, $beanFiles, $current_user;

        $error = new SoapError();
        if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', '', 'read', 'no_access',  $error)) {
            $GLOBALS['log']->info('End: checkSessionAndModuleAccess.');
            return;
        }

        //Querry Added by Umair khan on 5-11-2020
        $query = "SELECT contacts_cstm.id_c As contactID, contacts_cstm.portal_password_c As portal_pass,
            contacts.assigned_user_id AS AssignedUserID, contacts.first_name AS FirstName, contacts.last_name AS LastName,
            l1.email_address AS EmailAddress,
            ...
            LEFT JOIN  email_addr_bean_rel l1_1 ON contacts.id = l1_1.bean_id AND l1_1.deleted=0
            AND l1_1.bean_module = 'Contacts'  AND l1_1.primary_address = 1
            LEFT JOIN  email_addresses l1 ON l1.id=l1_1.email_address_id AND l1.deleted=0
            WHERE contacts_cstm.portal_password_c = '" . $parameters['password'] . "'  AND 
            l1.email_address='" . $parameters['user_name'] . "'
            AND contacts.deleted=0";

        $row = $GLOBALS['db']->query($query);
        $resStatus = $GLOBALS['db']->fetchByAssoc($row);
        return $resStatus;
    }

    public function crm_cases_lists($session, $parameters)
    {
        global  $beanList, $beanFiles, $current_user, $caseDetails;
        $error = new SoapError();
        if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', '', 'read', 'no_access',  $error)) {
            $GLOBALS['log']->info('End: checkSessionAndModuleAccess.');
            return;
        ...
        //Query changed by Umair Khan on 11-03-2020
        if ($accountID != " " && $accountID != "NULL") {
            $case_query = "SELECT 
        `cases`.case_number AS 'Number'
        , `cases`.id AS 'cases_id'
        , `cases`.name AS 'Subject'
        , `cases`.status AS 'Status'
        , `cases`.priority AS 'priority'
        , `cases`.date_entered AS 'date_entered'
        , `cases:accounts`.name AS 'Account_Name'
        , `cases:assigned_user_link`.first_name AS 'First_Name'
        , `cases:assigned_user_link`.last_name AS 'Last_Name'
        , `cases:assigned_user_link`.user_name AS 'user_name'
        , `cases:accounts`.id AS 'cases:accounts_id'
        , `cases:assigned_user_link`.id AS 'cases:assigned_user_link_id' 
        FROM `cases` 
        LEFT JOIN accounts `cases:accounts` ON `cases`.account_id=`cases:accounts`.id AND `cases:accounts`.deleted=0 
        LEFT JOIN users `cases:assigned_user_link` ON `cases`.assigned_user_id=`cases:assigned_user_link`.id AND `cases:assigned_user_link`.deleted=0 
        WHERE cases.account_id='$accountID' AND cases.deleted = 0 order by cases.date_entered desc";
        } else {
            $case_query = "SELECT 
            `cases`.case_number AS 'Number'
            , `cases`.id AS 'cases_id'
            ...
            LEFT JOIN accounts `cases:accounts` ON `cases`.account_id=`cases:accounts`.id AND `cases:accounts`.deleted=0 
            LEFT JOIN users `cases:assigned_user_link` ON `cases`.assigned_user_id=`cases:assigned_user_link`.id AND `cases:assigned_user_link`.deleted=0 
            WHERE cases.contact_created_by_id='$contactID' AND cases.deleted = 0 order by cases.date_entered desc";
        }
        $case_row = $GLOBALS['db']->query($case_query);
        while ($caseResut = $GLOBALS['db']->fetchByAssoc($case_row)) {
            $caseDetails[] = $caseResut;
        }
        return $caseDetails;
    }

    public function crm_userInfo($userID)
    {
        global  $beanList, $beanFiles, $current_user;
        $userSQL = "SELECT * FROM users where id = '" . $userID . "'";
        $userRows = $GLOBALS['db']->query($userSQL);
        while ($userResut = $GLOBALS['db']->fetchByAssoc($userRows)) {
            $userDetails[] = $userResut;
        }
     ...
     * **************************************************************/
    function crm_profile_details($session, $parameters)
    {
        global  $beanList, $beanFiles, $current_user;
        $error = new SoapError();
        $GLOBALS['log']->info('Under Profile Page Details');
        if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', '', 'read', 'no_access',  $error)) {
            $GLOBALS['log']->info('End: checkSessionAndModuleAccess.');
            return;
        }
        $query = "SELECT CONCAT( IFNULL( users.first_name,  '' ) ,  ' ', IFNULL( users.last_name,  '' ) ) as repoName, users.phone_work, emailAdd.email_address
                             FROM users 
							  LEFT JOIN email_addr_bean_rel AS emailRel ON emailRel.bean_id = users.id
							   LEFT JOIN email_addresses AS emailAdd ON emailAdd.id = emailRel.email_address_id
								WHERE users.id = '" . $parameters['usersID'] . "' AND users.employee_status = 'Active' 
								AND users.status='Active' AND emailAdd.deleted = 0 AND emailRel.deleted = 0";
        $rows = $GLOBALS['db']->query($query);
        while ($resuts = $GLOBALS['db']->fetchByAssoc($rows)) {
            $userinfo[] = $resuts;
        }
        return $userinfo;
    }
    ...
    /****************************************************************
     * organization Page Details by 
     * **************************************************************/
    function organization_profile_details($session, $parameters)
    {
        global $beanList, $beanFiles, $current_user;
        $error = new SoapError();
        if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', '', 'read', 'no_access', $error)) {
        ...
        //Changed By Umair Khan on 09-11-2020
        $query = "SELECT contacts_cstm.id_c As contactID
        ,contacts.assigned_user_id
        ,contacts.first_name AS firstname
        ,contacts.last_name AS lastname
        ,contacts.phone_mobile AS phone
        ,CONCAT( IFNULL( users.first_name,  '' ) ,  ' ', IFNULL( users.last_name,  '' ) ) as repoName
        ,users.phone_work AS repoPhone
        ,emailAdd.email_address AS RepEmailAddress
        ,l1.email_address AS emailAddress
        FROM contacts
        LEFT JOIN contacts_cstm on contacts.id = contacts_cstm.id_c
        ...
        $row = $GLOBALS['db']->query($query);
        $resStatus = $GLOBALS['db']->fetchByAssoc($row);
        $printed = print_r($resStatus, true);
        return $resStatus;
    }
    ...
    /****************************************************************
     * Update My Info Page Details byS
     * **************************************************************/
    function updateMyInfo_details($session, $parameters)
    {
        global $beanList, $beanFiles, $current_user;
        $error = new SoapError();
        if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', '', 'read', 'no_access', $error)) {
    ...
    //Start Added By Umair Khan On 9-11-2020
    function changePassword_func($session, $parameters)
    {
        global $beanList, $beanFiles, $current_user;
        $error = new SoapError();
        if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', '', 'read', 'no_access', $error)) {
        ...
        $contact_id = $parameters['contactID'];
        $querry = "SELECT contacts_cstm.id_c As ContactID
        ,contacts_cstm.portal_password_c As portalPassword
        From contacts
        LEFT JOIN contacts_cstm on contacts.id=contacts_cstm.id_c
        where contacts.id = '$contact_id'
        AND contacts.deleted=0";
        $row = $GLOBALS['db']->query($querry);
        $resStatus = $GLOBALS['db']->fetchByAssoc($row);
        return $resStatus;
    }
    function caseDetails($session, $parameters)
    {
        global $beanList, $beanFiles, $current_user;
        $error = new SoapError();
        if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', '', 'read', 'no_access', $error)) {
        ...
        LEFT JOIN accounts `cases:accounts` ON `cases`.account_id=`cases:accounts`.id AND `cases:accounts`.deleted=0 
        LEFT JOIN users `cases:assigned_user_link` ON `cases`.assigned_user_id=`cases:assigned_user_link`.id AND `cases:assigned_user_link`.deleted=0 
        WHERE cases.id = '$caseID' AND cases.deleted = 0";
        $case_row = $GLOBALS['db']->query($query);
        while ($caseResut = $GLOBALS['db']->fetchByAssoc($case_row)) {
            $caseDetails = $caseResut;
        }
        return $caseDetails;
    ...
    function caseUpdates($session, $parameters)
    {
        global $beanList, $beanFiles, $current_user;
        $error = new SoapError();
        $GLOBALS['log']->info('Under Profile Page Details');
        ...
        $caseID = $parameters['caseID'];
        $query = "SELECT aop_case_updates.id,
    aop_case_updates.name,
    aop_case_updates.date_entered,
    aop_case_updates.created_by,
    users.first_name,
    users.last_name,
    contacts.first_name,
    contacts.last_name,
    CONCAT( IFNULL( contacts.first_name,  '' ) ,  ' ', IFNULL( contacts.last_name,  '' ) ) AS c_fullname,
    CONCAT( IFNULL( users.first_name,  '' ) ,  ' ', IFNULL( users.last_name,  '' ) ) AS A_fullname
    FROM aop_case_updates
    LEFT JOIN users ON users.id = aop_case_updates.created_by
    LEFT JOIN contacts on aop_case_updates.contact_id = contacts.id
    WHERE aop_case_updates.case_id = '$caseID' 
    AND aop_case_updates.deleted = '0' 
    AND aop_case_updates.internal = '0' 
    order by aop_case_updates.date_entered desc ";
        $case_updates = $GLOBALS['db']->query($query);
        while ($caseResut = $GLOBALS['db']->fetchByAssoc($case_updates)) {
            $caseUpdates[] = $caseResut;
        }
        return $caseUpdates;
    }
    //End Added By Umair Khan On 9-11-2020
}
